...
let suite =
  "Analyze.Analyzer | Positive Operation"
  >::: [
    "integer primitive"
    >: (
      () =>
        Assert.analyzed_expression(
          AnalyzedUtil.(AST.Analyzed.(123 |> int_prim |> of_pos_op |> as_int)),
          RawUtil.(
            __empty_scope,
            AST.Raw.(123 |> int_prim |> of_pos_op |> as_raw_node),
          ),
        )
    ),
    "float primitive"
    >: (
      () =>
        Assert.analyzed_expression(
          AnalyzedUtil.(
            AST.Analyzed.((123.0, 3) |> float_prim |> of_pos_op |> as_float)
          ),
          RawUtil.(
            __empty_scope,
            AST.Raw.((123.0, 3) |> float_prim |> of_pos_op |> as_raw_node),
          ),
        )
    ),
    ...
    >: (
      () => {
        ...
        Assert.analyzed_expression(
          AST.Analyzed.(
            id |> as_weak |> of_id |> as_weak |> of_pos_op |> as_weak
          ),
          RawUtil.(
            scope,
            AST.Raw.(
