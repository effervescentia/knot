...
module FileStream2 = Knot.FileStream2;
...
let test_read_fully = (file, _) => {
  let channel = Util.load_resource2(file);
  let input = FileStream2.of_channel(channel);
...
let test_cursor_information = (file, char, position, row, column) => {
  ...
  _assert_cursor(
    _cursor_from(FileStream2.of_channel(channel), position),
    char,
    row,
    column,
  );
