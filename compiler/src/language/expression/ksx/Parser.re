...
let _attribute =
    (
      ...
      ctx: ParseContext.t,
      (parse_term, parse_expr): Parse.Kore.expression_parsers_arg_t,
    ) =>
  Matchers.assign(
...
let rec _inner_ksx =
        (ctx: ParseContext.t, parsers: Parse.Kore.expression_parsers_arg_t)
        : Parse.Kore.jsx_parser_t =>
...
and property_attribute =
    (ctx: ParseContext.t, parsers: Parse.Kore.expression_parsers_arg_t)
    : Parse.Kore.jsx_attribute_parser_t =>
...
and id_attribute: Parse.Kore.jsx_attribute_parser_t =
...
and attributes =
    (ctx: ParseContext.t, parsers: Parse.Kore.expression_parsers_arg_t)
    : Parse.Kore.jsx_attribute_list_parser_t =>
  choice([
...
and children =
    (ctx: ParseContext.t, parsers: Parse.Kore.expression_parsers_arg_t)
    : Parse.Kore.jsx_child_list_parser_t =>
  choice([node(ctx, parsers), inline_expr(ctx, parsers), text])
  |> Matchers.lexeme
  |> many

and text: Parse.Kore.jsx_child_parser_t =
...
and node =
    (ctx: ParseContext.t, parsers: Parse.Kore.expression_parsers_arg_t)
    : Parse.Kore.jsx_child_parser_t =>
  _inner_ksx(ctx, parsers) >|= Node.map(AST.Raw.of_node)

and inline_expr =
    (
      ctx: ParseContext.t,
      (_, parse_expr): Parse.Kore.expression_parsers_arg_t,
    )
    : Parse.Kore.jsx_child_parser_t =>
  parse_expr(ctx)
