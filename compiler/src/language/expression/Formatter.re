...
let rec pp_expression: Fmt.t(AST.Result.raw_expression_t) =
  ppf =>
    fun
    | Primitive(prim) => prim |> KPrimitive.pp(pp_expression, ppf)
    | Identifier(name) => name |> KIdentifier.pp(pp_expression, ppf)
    ...
    | Closure(stmts) => stmts |> KClosure.pp(pp_expression, ppf)
    | DotAccess(expr, prop) =>
