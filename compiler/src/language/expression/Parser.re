...
let primitive: Parse.Kore.expression_parser_t =
  KPrimitive.Plugin.parse >|= Node.map(AST.Raw.of_prim);

let identifier = (ctx: ParseContext.t): Parse.Kore.expression_parser_t =>
...
/* || */
let rec expr_0 = (ctx: ParseContext.t): Parse.Kore.expression_parser_t =>
  chainl1(expr_1(ctx), KLogicalOr.Plugin.parse)

/* && */
and expr_1 = (ctx: ParseContext.t): Parse.Kore.expression_parser_t =>
  chainl1(expr_2(ctx), KLogicalAnd.Plugin.parse)

/* ==, != */
and expr_2 = (ctx: ParseContext.t): Parse.Kore.expression_parser_t =>
  chainl1(expr_3(ctx), KEqual.Plugin.parse <|> KUnequal.Plugin.parse)

/* <=, <, >=, > */
and expr_3 = (ctx: ParseContext.t): Parse.Kore.expression_parser_t =>
  chainl1(
    expr_4(ctx),
    choice([
...
/* +, - */
and expr_4 = (ctx: ParseContext.t): Parse.Kore.expression_parser_t =>
  chainl1(expr_5(ctx), KAdd.Plugin.parse <|> KSubtract.Plugin.parse)

/* *, / */
and expr_5 = (ctx: ParseContext.t): Parse.Kore.expression_parser_t =>
  chainl1(expr_6(ctx), KMultiply.Plugin.parse <|> KDivide.Plugin.parse)

/* ^ */
and expr_6 = (ctx: ParseContext.t): Parse.Kore.expression_parser_t =>
  chainr1(expr_7(ctx), KExponentiate.Plugin.parse)

/* !, +, - */
and expr_7 = (ctx: ParseContext.t): Parse.Kore.expression_parser_t =>
  expr_8(ctx) |> KUnaryOperator.Plugin.parse

/* foo(bar) */
and expr_8 = (ctx: ParseContext.t): Parse.Kore.expression_parser_t =>
...
/* foo.bar */
and expr_9 = (ctx: ParseContext.t): Parse.Kore.expression_parser_t =>
  expr_10(ctx) >>= KDotAccess.Plugin.parse

/* {}, () */
and expr_10 = (ctx: ParseContext.t): Parse.Kore.expression_parser_t =>
  /* do not attempt to simplify this `input` argument away or expression parsing will loop forever */
  input =>
    choice(
      [
...
and jsx_term = (ctx: ParseContext.t): Parse.Kore.expression_parser_t =>
...
/* 2, foo, <bar /> */
and term = (ctx: ParseContext.t): Parse.Kore.expression_parser_t =>
  choice([
    primitive,
