...
type import_spec_t = (
  ...
  option(string),
  list((string, option(AST.Result.identifier_t))),
);

let pp_namespace: Fmt.t(Reference.Namespace.t) =
  ppf => ~@Reference.Namespace.pp % KString.Plugin.pp(ppf);

let pp_named_import: Fmt.t((string, option(AST.Result.identifier_t))) =
  ppf =>
    fun
...
let pp_named_import_list:
  Fmt.t(list((string, option(AST.Result.identifier_t)))) =
