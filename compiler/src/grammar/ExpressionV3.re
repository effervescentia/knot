...
let identifier = (ctx: ModuleContext.t): expression_parser_t =>
  Identifier.parser(ctx) >|= N.wrap(AR.of_id);

let jsx =
    (
      ctx: ModuleContext.t,
      parse_term: contextual_expression_parser_t,
      parse_expr: contextual_expression_parser_t,
    )
    : expression_parser_t =>
  JSX.parser(ctx, (parse_term, parse_expr)) >|= N.wrap(AR.of_jsx);
...
and expr_8 = (ctx: ModuleContext.t): expression_parser_t =>
  choice([closure(ctx, expr_0), expr_0(ctx) |> group, term(ctx)])
