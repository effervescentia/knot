...
let _attribute =
  ...
  >|= (
    ((name, expr)) => (
      name,
      Some(expr),
      Range.join(NR.get_range(name), N.get_range(expr)),
    )
  )
        ...
        attrs =>
          ...
          >>= (
            cs =>
              ...
              >|= snd
              >|= (end_range => NR.create(cs, end_range))
          )
          <|> _self_closing
          >|= (
            cs =>
...
and identifier_attribute: jsx_attribute_parser_t =
...
and children =
  ...
  choice([node(ctx, parsers), text]) |> M.lexeme |> many
