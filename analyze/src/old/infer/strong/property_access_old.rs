    ...
    match strong.get_deep_type(lhs, node.kind()) {
        Some(Ok(x @ Type::Module(declarations))) => {
            match declarations.iter().find(|(name, ..)| name == property) {
                ...
                {
                    strong
                        .get_type(declaration_id, node.kind())
                        .map(partial::Action::Infer)
                        .unwrap_or(partial::Action::Skip)
                }

                Some((_, declaration_kind, _)) => partial::Action::Infer(&Err(
                    SemanticError::UnexpectedKind((*declaration_kind, *lhs), *node.kind()),
                )),

                None => partial::Action::Infer(&Err(SemanticError::DeclarationNotFound(
                    (x.to_shallow(), *lhs),
                    property.to_string(),
                ))),
            }
        ...
        Some(Ok(x @ Type::Enumerated(Enumerated::Declaration(variants)))) => {
            match variants.iter().find(|(name, _)| name == property) {
                ...
                None => partial::Action::Infer(&Err(SemanticError::VariantNotFound(
                    (x.to_shallow(), *lhs),
                    property.to_string(),
                ))),
            }
        ...
        Some(Ok(x)) => partial::Action::Infer(&Err(SemanticError::NotIndexable(
            (x.to_shallow(), *lhs),
            property.to_string(),
        ))),
