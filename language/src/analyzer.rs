...
use crate::declaration::{Declaration, Parameter, Storage};
use crate::expression::{BinaryOperator, Expression, UnaryOperator};
use crate::import::{Import, Target};
use crate::ksx::KSX;
use crate::module::Module;
use crate::primitive::Primitive;
use crate::statement::Statement;
use crate::types::type_expression::TypeExpression;

struct Context<'a> {
    types: &'a RefCell<HashMap<usize, WeakType>>,
    scope: HashMap<String, usize>,
}

impl<'a> Context<'a> {
    fn new(types: &'a RefCell<HashMap<usize, WeakType>>) -> Self {
        Self {
            types,
            scope: HashMap::new(),
        }
...
fn view(parameters: Vec<Parameter>, body: Expression, ctx: Context) -> (usize, Context) {
    let child_ctx = ctx.child();
    let (param_ids, child_ctx) =
        parameters
            .into_iter()
...
fn module(imports: Vec<Import>, declarations: Vec<Declaration>, ctx: Context) -> (usize, Context) {
    let child_ctx = ctx.child();
...
fn declaration<'a>(x: Declaration, ctx: Context) -> (String, usize, Context) {
